...
rec {
  ...
  extendModule =
    ...
    let
      eval =
        if (builtins.isString path) || (builtins.isPath path) then
          import path moduleArgs
        else
          path moduleArgs;

      evalOnlyConfig = builtins.removeAttrs eval [
        "imports"
        "options"
      ];
      ...
      extra =
        if
          (builtins.hasAttr "extraOptions" extenderArgs) || (builtins.hasAttr "extraConfig" extenderArgs)
        then
          [
            (
              { ... }:
    ...
    in
    {
      ...
      options =
        if builtins.hasAttr "optionsExtension" extenderArgs then
          (extenderArgs.optionsExtension (eval.options or { }))
        else
          (eval.options or { });

      config =
        if builtins.hasAttr "configExtension" extenderArgs then
          (extenderArgs.configExtension (eval.config or evalOnlyConfig))
        else
          (eval.config or evalOnlyConfig);
    };
